  ...
  passport.use(new GithubStrategy({
    ...
    process.nextTick(function() {
      if (req.session.authAction === "login") {
        ...
        if (req.user) {
          // Connecting
          return req.user.linkProvider("github", token, profile)
            .then(function(user) {
              done(null, user, "Successfully attached Github to your user account");
            }, function(error) {
              done(error);
            });
        } else {
          // Signup
          return User.createFromProvider("github", token, profile)
            .then(function(user) {
              done(null, user);
            }, function(error) {
              done(error);
